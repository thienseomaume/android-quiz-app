...
public class ResultsAdminDetailed extends AppCompatActivity {
    ...
    public void saveToExcel(String fileName) {
        ...
        while (i < this.testAdapter.dataList.size()) {
            ...
            createRow2.createCell(0).setCellValue(this.testAdapter.dataList.get(i).user.nickName);
            createRow2.createCell(1).setCellValue(this.testAdapter.dataList.get(i).user.fullName);
            createRow2.createCell(2).setCellValue(this.testAdapter.dataList.get(i).user.address);
            createRow2.createCell(3).setCellValue(this.testAdapter.dataList.get(i).user.number);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (int i=0;i<result.size();i++){
                    if(dataSnapshot.child(result.get(i).userID).exists())
                        ...
                        user1.setAddress("Details not added yet");
                        user1.setNumber("Details not added yet");
                        user1.setFullName("Details not added yet");
    ...
    class TestAdapter extends ArrayAdapter<TestResults> {
        ...
        public View getView(final int position, @Nullable View convertView, @NonNull ViewGroup parent) {
            ...
            if(dataList.get(position).user!=null)
                ((TextView)listItem.findViewById(R.id.item_textView)).setText(dataList.get(position).user.nickName);
            else {
            ...
            if(isAdmin ) {
                ((TextView) listItem.findViewById(R.id.item_textView)).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (dataList.get(position).user.nickName != null) {
